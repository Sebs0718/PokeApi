...
function Pokemons (){
    ...
    const getPokemons = async ()=>{
        try {
            const { data } = await Axios.get(`https://pokeapi.co/api/v2/pokemon/?limit=1119`);
        ...
        <div className='page'>
            ...
            <div className={sidebar ? 'content-page' : 'content-page-desactive'} >
                ...
                <div className='container body'>
                    <div className='row'>
                        <h6 className='offset-md-1 col-md-6'>Busqueda de Pokémons</h6>
                        <input type='text' className='col-md-8 offset-md-1' id='search' placeholder='Buscar' onChange={getValueSearch} value={search}/>
                        <FlastList 
                            list={search != "" ? pokemons.filter((text) =>  text.name.toLowerCase().includes(search)) : pokemons}
                            renderItem={(item,index)=>{
                                if(search != ""){
                                    return(
                                        <div className='col-md-4 my-3' key={index}>
                                            <div className='card'>
                                                <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${item.id}.png`} alt='img' />
                                                <h6>{item.name}</h6>
                                                <Link to={`/PokemonMore/${item.id}`} >
                ...
                <div className='container footer my-2'>
                    ...
                    <button className='btn btn-sm btn-primary mx-2' onClick={pagintate < 1110 ?nextPaginate : ()=>{}} disabled={search != "" ? true : false}> <AiIcons.AiOutlineArrowRight /> </button>
                </div>
            </div >
        </div>
    );
